...
// static DriversState driversState[5] = {0,0,0,0,0};
...
{
public:
	void Init(uint32_t p_driverNumber, Pin csPin) noexcept;
...
void TmcDriverState::Init(uint32_t p_driverNumber, Pin csPin) noexcept
pre(!driversPowered)
{
	_csPin = csPin;
...
void RxDmaCompleteCallback(CallbackParameter param) noexcept
{
	...
	{
		fastDigitalWriteHigh(CS_PINS[i]);
	}
	tmcTask.GiveFromISR();
}
		...
		else
		{
			...
			else if (!timedOut)
			{
				...
				if (driversState == DriversState::initialising)
				{
					...
					if (driverStates[drive].UpdatePending())
					{
			...
			driverStates[drive].GetSpiCommand(writeBufPtr);
			...
			// Unfortunately, when we disable SPI the SCLK line floats. Therefore we disable SPI for as little time as possible.
			{
				...
				fastDigitalWriteLow(CS_PINS[drive]);			// set CS low
			...
			if (timedOut)
			{
				...
				fastDigitalWriteHigh(CS_PINS[drive]);			// set CS high
...
void SmartDrivers::Init() noexcept
{
	...
	{
		driverStates[driver].Init(driver, CS_PINS[driver]);
	}
...
//void SmartDrivers::Diagnostics(MessageType mtype) noexcept
//{
//	platform->Message(mtype, "=== Drivers ===\n");
//	platform->Message(mtype, "Microstepping - ");
//	for (size_t axis = 0; axis < numTotalAxes; ++axis)
//	{
//		bool actualInterp;
//		const unsigned int microsteps = platform.GetMicrostepping(axis, actualInterp);
//		platform->Message(mtype, "%c:%u%s, ", 	[axis], microsteps, (actualInterp) ? "(on)" : "");
//	}
//	platform->Message(mtype, "E");
